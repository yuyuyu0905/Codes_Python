...
def bfs2(start_point):
    print 'creating commuity start at :',start_point
    start=[start_point]
    finish=[start_point]
    edges={}
    can_go=True
...
while True:
    ...
    while status:
        ...
        while status:
            ...
            index += 1
            ds[maxtuple[0]] = 0
            dd[maxtuple[0][0]].remove(maxtuple[0][1])
            dd[maxtuple[0][1]].remove(maxtuple[0][0])
            status = bfs_findpath(dd, maxtuple[0][0], maxtuple[0][1])
        for key in component:
            if maxtuple[0][0] in component[key] and maxtuple[0][1] in component[key]:
                component.pop(key)
                print 'pop key:',key
                break
        component[maxtuple[0][0]] = bfs2(maxtuple[0][0])
        component[maxtuple[0][1]] = bfs2(maxtuple[0][1])
    if index==ld:break
    for i in component:
        l = len(component[i])
        for x in range(l):
            for y in range(x + 1, l):
                if (component[i][x], component[i][y]) in d or (component[i][y], component[i][x]) in d:
                    result = 1 - (0.0 + degree_dict[component[i][x]] * degree_dict[component[i][y]]) / (2 * m)
                else:
                    result = -(0.0 + degree_dict[component[i][x]] * degree_dict[component[i][y]]) / (2 * m)
                count += result
    print 'current modularity:',count / (2 * m),'max modularity:',maxmodularity
    currentmodularity=count/(2*m)
    if index==ld:break
    else:
        maxmodularity = max(maxmodularity, currentmodularity)
    if maxmodularity==currentmodularity:
        maxcomponent=component.copy()

file=open(outputpath, 'w')
maxcomponent2={}
for x in maxcomponent:
    maxcomponent[x].sort()
    maxcomponent2[maxcomponent[x][0]] = maxcomponent[x]
maxcomponent3=sorted(maxcomponent2.iteritems(),key=lambda x:x[0])
for x in maxcomponent3:
    file.write(str(x[1]) + '\n')
